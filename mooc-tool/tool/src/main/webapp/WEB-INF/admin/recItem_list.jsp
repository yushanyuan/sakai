...
<table  class="listHier lines nolines">
	<tr><th>课程名</th><th>推荐值</th><th>专业</th><th>图片</th><th>老师</th><th>操作</th></tr>
	<c:forEach items="${list }" var="c">
		<tr><td>${c.course.name }</td><td>${c.recValue }</td><td>${c.course.category.name }</td><td><img width="80px" height="80px" src="${moocServer }${c.course.image }"/></td><td>${c.course.teacher.name }</td><td><a href="remove.htm?id=${c.id }">[删除]</a></td></tr>
	</c:forEach>
</table>
<ul id="actionToolBar" class="navIntraTool actionToolBar">
	<li class="firstToolBarItem"><span><a href="add.htm">添加推荐课程</a></span></li>
</ul>
